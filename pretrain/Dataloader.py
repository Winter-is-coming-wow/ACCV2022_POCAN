...
class DataLoader:
    ...
    def dataset_parser_sup(self, value, cluster_label=None): #for source dataset
        ...
        if self.task!='pretext':
            ...
            label=self.translabel(self.args.source,label)
            return (inputs, label)
        else:
            img = value
            label=cluster_label
...
class JAFFEdataset:
    def __init__(self):
        ...
        self.image_path = r'G:\deeplearning\FER datasets\JAFFE\jaffe\jaffe'
    ...
    def load_data(self):
        ...
        for dir in os.listdir(self.image_path):
            for file in os.listdir(os.path.join(self.image_path, dir)):
                image = cv.imread(os.path.join(os.path.join(self.image_path, dir), file), 0)
        ...
        # print(sum(class_num))
        # plt.bar(self.label2index.keys(), class_num)
        # plt.savefig('datasets/JAFFE.jpg')
        return np.asarray(images)[index], np.asarray(labels)[index]
...
class CKplus:
    def __init__(self):
        ...
        self.write_dir = r'/home/njustguest/wangchao/datasets/ckplus/aligned'
        self.label_path = r'/home/njustguest/wangchao/datasets/ckplus/label_ck.txt'
    ...
    def load_data(self):
        ...
        for dir in os.listdir(self.write_dir):
            for file in os.listdir(os.path.join(self.write_dir, dir)):
                image = cv.imread(os.path.join(self.write_dir, dir, file))
...
class SFEW:
    def __init__(self, mode=None):
        ...
        self.train_images_path = r'G:\deeplearning\FER datasets\SFEW\processed\train_aligned'
        self.val_images_path = r'G:\deeplearning\FER datasets\SFEW\processed\val_aligned'
        ...
        self.train_tfrecord_path = r'G:\demo\cross_dataset\datasets\sfew_train_tfrecord'
        self.val_tfrecord_path = r'G:\demo\cross_dataset\datasets\sfew_val_tfrecord'
            ...
            for emotion in os.listdir(boxes_path):
                for txt_name in os.listdir(os.path.join(boxes_path, emotion)):
                    ...
                    if os.path.exists(os.path.join(raw_path, emotion, txt_name[:-3] + 'png')):
                        image = cv.imread(os.path.join(raw_path, emotion, txt_name[:-3] + 'png'), 0)
                    elif os.path.exists(os.path.join(raw_path, emotion, txt_name[:-3] + 'jpg')):
                        image = cv.imread(os.path.join(raw_path, emotion, txt_name[:-3] + 'jpg'), 0)
                    else:
...
class oulu_CASIA:
    def __init__(self):
        self.root_dir = r'/home/njustguest/wangchao/datasets/oulu/Strong'
        self.write_dir = r'/home/njustguest/wangchao/datasets/oulu/Strong_processed'
...
class ExpW:
    def __init__(self):
        self.raw_data = r'/home/njustguest/wangchao/datasets/ExpW/data/image/data'
        self.write_path = r'/home/njustguest/wangchao/datasets/ExpW/data/image/processed'
        self.label_path = r'/home/njustguest/wangchao/datasets/ExpW/data/label/label.lst'

        self.tfrecord_path = r'G:\demo\cross_dataset\datasets'
        self.rgb_tfrecord_path = r'G:\demo\cross_dataset\datasets\ExpW_tfrecord'
        self.gray_tfrecord_path = r'G:\demo\cross_dataset\datasets\ExpW_gray_tfrecord'
    ...
    def load_data(self, channel=1, type=2):
        """
        利用分层五折交叉将数据集分成5份。
        :param channel: channel of image
        :param type: if equal 0 return rgb dataset,elif equal 1 return gray dataset,else return divided dataset
        :return: a list which contain 5 tf.data.Dataset type data.
        """
        if type == 0:  # 返回3通道图片
            feature_description = {
                'image': tf.io.FixedLenFeature([], tf.string),
            ...
            raw_dataset = tf.data.TFRecordDataset(self.rgb_tfrecord_path)
            ...
            raw_dataset = tf.data.TFRecordDataset(self.gray_tfrecord_path)
            dataset = raw_dataset.map(_parse_example)
            return dataset
        ...
        part_id = 1
        datasets = []
        for train, test in skf.split(images_path[:, 0], images_path[:, 1]):
            part_path = images_path[test]
            part_tfrecord_path = os.path.join(self.tfrecord_path, "part{}_ExpW_tfrecord".format(part_id))
            part_id += 1
            if not os.path.exists(part_tfrecord_path):
                with tf.io.TFRecordWriter(part_tfrecord_path) as writer:
                    for item in part_path:
            ...
            raw_dataset = tf.data.TFRecordDataset(part_tfrecord_path)
            dataset = raw_dataset.map(_parse_example)
            datasets.append(dataset)
            print("part  {} done".format(part_id))
        return datasets
        ...
        with open(self.label_path) as f:
            for line in f:
                ...
                if image is None:
                    ...
                    if face_image is None:
                        c+=1
                        print("mtcnn detect no face in {}, dlib will be used".format(os.path.join(self.raw_data, image_name)))
...
class AffectNet:
    def __init__(self):
        ...
        self.tfrecord_dir = r'G:\demo\cross_dataset\datasets\AffectNet'
        ...
        self.train_image_num = 414798  # [74874, 134415, 25459, 14090, 6378, 3803, 24882, 3750, 33088, 11645, 82414]
        self.val_image_num = 5500  # [500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500]

    def load_data(self, mode=0, channel=1):
        feature_description = {
            'image': tf.io.FixedLenFeature([], tf.string),
            'label': tf.io.FixedLenFeature([], tf.int64)
        }
