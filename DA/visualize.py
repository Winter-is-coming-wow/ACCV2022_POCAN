...
from Dataloader import Fer2013dataset,Fer2013Plus,RAFdataset,ExpW,CKplus,DataLoader,oulu_CASIA,JAFFEdataset,RAFdataset2
import cv2,os
...
def Visualization(net, datasetName,args,with_proto=False):
    ...
    if datasetName == 'fer2013':
        ...
        for feature in pred:
            Feature.append(feature)
        for image, label in train_ds:
            Label.append(transer('JAFFE', label))
    
    if(with_proto):
        proto_feature=calculate_prototype(np.array(Feature),np.array(Label,dtype=np.float64))
    ...
    if with_proto:
        colors = {0: 'tomato', 1: 'cornflowerblue', 2: 'lawngreen', 3: 'cyan', 4: 'orange', 5: 'purple', 6: 'pink',
    ...
    plt.savefig(fname='ablation/{}_{}_beforeda.png'.format(args.source,datasetName,args.train_info), bbox_inches='tight')
...
def VisualizationForTwoDomain(net, source_dataset, target_dataset,args,with_proto=False):
    ...
    if target_dataset == 'fer2013':
        ...
        for feature in pred:
            Feature_Target.append(feature)
        for image, label in train_ds:
            Label_Target.append(transer('JAFFE', label))
    ...
    if with_proto:
        ...
        labels = {0: 'Anger', 1: 'Disgust', 2: 'Fear', 3: 'Happiness', 4: 'Sadness', 5: 'Surprise', 6: 'Neutral', \
                7: 'Anger', 8: 'Disgust', 9: 'Fear', 10: 'Happiness', 11: 'Sadness',12: 'Surprise', 13: 'Neutral'}
        colors = {0: 'lightblue', 1: 'lightcoral', 2: 'bisque', 3: 'lightgreen', 4: 'plum', 5: 'silver', 6: 'pink', \
                  7: 'blue',      8: 'red',        9: 'orange', 10: 'green',    11: 'purple',12: 'black', 13: 'deeppink'}
    ...
    for i in range(14):
        ...
        if i < 7:
            scatter = plt.scatter(data_x, data_y, c=colors[i], s=5, label=labels[i], marker='o',alpha=0.4)
        elif i<14:
            scatter = plt.scatter(data_x, data_y, c=colors[i], s=5, label=labels[i], marker='v',alpha=0.9)
        elif i<21:
    ...
    l1 = plt.legend(handles=[mpatches.Patch(color=colors[i], label="{:s}".format(labels[i])) for i in range(7)],
                    loc='upper left', prop={'size': 9}, borderaxespad=0, title='source', bbox_to_anchor=(0.91, 0.80),
                    bbox_transform=fig.transFigure)
    l2 = plt.legend(handles=[mpatches.Patch(color=colors[i + 7], label="{:s}".format(labels[i + 7])) for i in range(7)],
                    loc='upper left', prop={'size': 9}, borderaxespad=0, title='target', bbox_to_anchor=(0.91, 0.46),
                    bbox_transform=fig.transFigure)

    l3 = plt.legend([source, target], ['Source Domain', 'Target Domain'], loc='upper left', prop={'size': 8},
                    borderaxespad=0, bbox_to_anchor=(0.91, 0.88), bbox_transform=fig.transFigure)
    plt.gca().add_artist(l3)
    plt.gca().add_artist(l1)
    plt.gca().add_artist(l2)

    plt.savefig(fname='{}_{}_{}_1.png'.format(args.source, source_dataset, target_dataset), bbox_inches='tight')
...
def main():
    ...
    logger.info("------------------------------------------------------------------")
    args.encoder_snapshot=r'C:\Users\DELL\PycharmProjects\superwang\final\DA\ablation\RAF_ckplus\RAF_ckplus_resnet50_20_sgdw_noem_0\backbone'
    args.proj_snapshot = r'C:\Users\DELL\PycharmProjects\superwang\final\DA\ablation\RAF_ckplus\RAF_ckplus_resnet50_20_sgdw_noem_0\proj'
    ...
    #Visualization(model,'ckplus',args)
    VisualizationForTwoDomain(model,'RAF','ckplus',args)
    #VisualizationForPreDomain(model,'RAF','fer2013',args)
...
if __name__ == '__main__':
    #main()
    import cv2 as cv
    img=cv.imread(r'C:\Users\DELL\PycharmProjects\superwang\final\DA\RAF_RAF_ckplus_2.png')
    img=cv.flip(img,0)
    cv.imwrite('./RAF_RAF_ckplus_3.png',img)