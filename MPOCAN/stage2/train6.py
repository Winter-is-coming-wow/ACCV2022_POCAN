...
def train(sources, target, args, logger, output_path):
    ...
    def embedding_net(source=True):
        ...
        if source:
            ...
            for proj, classifier in zip(net.proj, net.source_classifier):
                ...
                logit = classifier(z, training=False)
                logits.append(logit)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.interval == 0 or epoch == args.epochs - 1:
            ...
            #calculate source weight and calculate source weight##########################################################################
            classifier = embedding_net(True)
            output1 = classifier.predict(target_train_dataset_evaluate)
            for p in range(len(args.sources)):
                one = np.asarray(output1)[p, :, :]
                pse_label = np.argmax(one, axis=-1)
                acc_i = np.sum(pse_label == Label_Target) / Label_Target.shape[0]
                logger.info(
                    "evaluate target train data with source classifier {} at epoch {},   acc : {}!".format(p, epoch,
                                                                  acc_i))
            logger.info("=")
            classifier = embedding_net(False)
            output2 = classifier.predict(target_train_dataset_evaluate)
            domain_e=[]
            for p in range(len(args.sources)):
                one = np.asarray(output2)[p, :, :]
                one = softmax(one)
                pse_label = np.argmax(one, axis=-1)
                conf = [one[i][pse_label[i]] for i in range(len(pse_label))]
                domain_e.append(np.sum(np.asarray(conf)) / pse_label.shape[0])
                logger.info("domain_e:{}".format(domain_e))
                acc_i = np.sum(pse_label == Label_Target) / Label_Target.shape[0]
                logger.info(
                    "evaluate target train data with target classifier {} at epoch {},   acc : {}!".format(p, epoch,
                                                                                                           acc_i))
            domain_e=np.asarray(domain_e)
            domain_e=np.reshape(domain_e,(-1,1))
            sw = softmax(np.concatenate([output1,output2], axis=-1))
            sw1,sw2=np.split(sw,2,axis=-1)
            #sw1 = np.sum(sw1, axis=-1)
            sw = np.sum(sw2,axis=-1)

            sw = np.log(sw)
            sw = -sw
            sw *= domain_e
            sw = sw.T
            sw/=np.sum(sw,axis=-1,keepdims=True)
            sw=sw.T

            logger.info("sw:{}".format(sw.T))
            sw=np.expand_dims(sw,-1)
            output=softmax(np.asarray(output2))
            output *= sw
            target_logits = np.sum(output, 0)
            #print(target_logits)
            target_logits /= np.expand_dims(np.sum(target_logits,axis=1),-1)
            #print(target_logits)
            ...
            logger.info('target class weight{}'.format(target_class_weight))
            ...
            classifier = embedding_net(True)
            output1 = classifier.predict(target_test_dataset)
            for p in range(len(args.sources)):
                one = np.asarray(output1)[p, :, :]
                one = np.argmax(one, axis=-1)
                acc_i = np.sum(one == Label_Target_test) / Label_Target_test.shape[0]
                logger.info(
                    "evaluate target test data with source classifier {} at epoch {},   acc : {}!".format(p, epoch,
                                                                                                          acc_i))
            classifier = embedding_net(False)
            output2 = classifier.predict(target_test_dataset)
            for p in range(len(args.sources)):
                one = np.asarray(output2)[p, :, :]
                one = np.argmax(one, axis=-1)
                acc_i = np.sum(one == Label_Target_test) / Label_Target_test.shape[0]
    ...
    for j in range(len(args.sources)):
        net.proj[j].save(os.path.join(output_path, "proj_{}".format(args.sources[j])))
