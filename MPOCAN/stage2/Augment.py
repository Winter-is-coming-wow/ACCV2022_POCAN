...
class Augment:
    ...
    def _augment_pretext(self, x, shape, coord=[[[0., 0., 1., 1.]]]):
        ...
        x = self._resize(x)
        x = self.random_resize_crop(x, 0.8, 0.95, shape[0])
        #x = self.random_resize_crop(x,0.8,0.95,shape[0])
    ...
    def random_resize_crop(self, image, min_scale, max_scale, crop_size):
        ...
        if crop_size == 224:
            image_shape = 260
            image = self._resize(image, (image_shape, image_shape))
        else:
            image_shape = 120
            image = self._resize(image, (image_shape, image_shape))
        ...
        crop_resize = self._resize(crop, (crop_size, crop_size))
...
if __name__ == '__main__':
    ...
    image_path = r'G:\superwang\SFEW2.0\train_aligned\Angry\Airheads_000519240_00000005.png'
    image = cv.imread(image_path)
    print(image.shape)
    # image = np.expand_dims(image, -1)
    cv.imshow('a', image)
    cv.waitKey(0)
    g = Augment(Config()).center_crop(image,256,256,0.85)
