    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.interval == 0 or epoch == args.epochs - 1:
            ...
            for p in range(len(args.sources)):
                ...
                for i in range(pse_label.shape[0]):
                    class_num[pse_label[i]]+=1
                    class_e[pse_label[i]]+=conf[i]
                domain_e = np.sum(class_e) / np.sum(class_num)
                logger.info("domain_e:{}".format(domain_e))
                class_e=np.divide(np.asarray(class_e),np.asarray(class_num),out=np.zeros_like(np.asarray(class_e),dtype=np.float64), where=np.asarray(class_num)!=0)
                class_e*=domain_e
                source_weight.append(class_e.tolist())
            ...
            source_weight=np.asarray(source_weight)
            source_weight/=np.sum(source_weight,axis=0)
            logger.info("source weight : {}".format(source_weight))
            source_weight=np.reshape(source_weight,(len(args.sources),1,7))
            logger.info("source weight : {}".format(source_weight))
            #select trust target samples
            ...
            target_train_ds_trust = target_train_ds.filter(
                lambda x, y, z: z >= tf.cast(tf.gather_nd(confidence, tf.reshape(y, (1,))), tf.float64))
